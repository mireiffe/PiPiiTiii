<script>
    export let variant = "primary"; // primary, secondary, destructive, ghost, outline
    ...
    $: classes = `${baseClasses} ${variants[variant] || variants.primary} ${sizes[size] || sizes.md} ${$$props.class || ""}`;
</script>
...
{:else}
    <button {type} class={classes} {disabled} {title} on:click {...$$restProps}>
        {#if loading}
            <svg
