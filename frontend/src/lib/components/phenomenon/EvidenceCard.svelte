<script lang="ts">
    ...
    export let color: { bg: string; border: string; name: string } | undefined =
        undefined;
...
<div
    ...
    style={color
        ? `border-color: ${color.border}20; background: ${color.bg.replace("0.2", "0.05")};`
        : ""}
    role="listitem"
...
>
    {#if isCaptureEvidence(evidence)}
        <div class="flex items-start gap-2">
            ...
            <div class="flex-1 min-w-0">
                ...
                {:else}
                    <button
                        class="mt-0.5 text-xs text-left cursor-pointer hover:text-blue-600 leading-snug break-all"
                    ...
                    >
                        ...
                        <span class="text-gray-400 font-normal ml-1">
                            [슬라이드 {evidence.slideIndex + 1} - {Math.round(
                                evidence.width,
                            )}x{Math.round(evidence.height)} ({Math.round(
                                evidence.x,
                            )}, {Math.round(evidence.y)})]
                        </span>
                    </button>
                {/if}
            ...
            <button
                class="flex-shrink-0 opacity-0 group-hover:opacity-100 p-1 text-gray-400 hover:text-red-500 transition-all mt-0.5"
    ...
    {:else if isAttributeEvidence(evidence)}
        <div class="flex items-start gap-2">
            <div
                class="flex-shrink-0 w-5 h-5 rounded bg-purple-100 flex items-center justify-center"
            >
                <svg
