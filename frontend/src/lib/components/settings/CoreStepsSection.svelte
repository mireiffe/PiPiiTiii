<script lang="ts">
    ...
    function removeStep(stepId: string) {
        if (confirm("이 Core Step을 삭제하시겠습니까?")) {
            coreSteps.definitions = coreSteps.definitions.filter(d => d.id !== stepId);
    ...
    function handlePresetNameChange(stepId: string, presetId: string, name: string) {
        const step = coreSteps.definitions.find(d => d.id === stepId);
        if (step) {
    ...
    function togglePresetType(stepId: string, presetId: string, type: CoreStepInputType) {
        const step = coreSteps.definitions.find(d => d.id === stepId);
        if (step) {
            const preset = step.presets.find(p => p.id === presetId);
            if (preset) {
    ...
    <div class="flex justify-between items-center mb-4">
        <div>
            ...
            <p class="text-sm text-gray-500 mt-1">
                Core Step은 Global Step과 독립적으로 관리됩니다.
                각 Core Step에는 필수 입력 필드(Preset)를 정의할 수 있습니다.
            </p>
        </div>
            ...
            {#if addingStep}
                <div class="flex items-center gap-2">
                    ...
                    <button
                        ...
                        on:click={() => { addingStep = false; newStepName = ""; }}
                    >
                        취소
                    </button>
                </div>
    ...
    <div class="space-y-3">
        ...
        {:else}
            {#each coreSteps.definitions as step, index (step.id)}
                <div class="border border-purple-200 rounded-lg bg-purple-50/50 overflow-hidden">
                    <!-- Step Header -->
                    <div class="flex items-center gap-3 p-4">
                        <div class="flex flex-col gap-1">
                            ...
                            <button
                                ...
                                disabled={index === coreSteps.definitions.length - 1}
                                title="아래로 이동"
                            >
                                ▼
                            </button>
                        ...
                        <div class="flex-1">
                            <input
                                type="text"
                                value={step.name}
                                on:input={(e) => handleStepNameChange(step.id, e.currentTarget.value)}
                        ...
                        <button
                            ...
                            on:click={() => dispatch("toggleStepExpand", { stepId: step.id })}
                            title="상세 편집"
                        >
                            ...
                            <div class="mb-4">
                                ...
                                <div class="space-y-2 mb-4">
                                    {#if step.presets.length === 0}
                                        <div class="text-sm text-gray-400 py-4 text-center border border-dashed border-gray-300 rounded">
                                            정의된 Preset이 없습니다.
                                        </div>
                                    {:else}
                                            ...
                                            <div class="flex items-center gap-3 p-3 bg-gray-50 rounded-lg border border-gray-200">
                                                <div class="flex flex-col gap-0.5">
                                                    <button
                                                        ...
                                                        on:click={() => movePresetUp(step.id, pIndex)}
                                                        disabled={pIndex === 0}
                                                    >
                                                        ▲
                                                    </button>
                                                    <button
                                                        class="text-gray-400 hover:text-gray-600 disabled:opacity-30 text-xs"
                                                        on:click={() => movePresetDown(step.id, pIndex)}
                                                        disabled={pIndex === step.presets.length - 1}
                                                    >
                                                        ▼
                                                    </button>
                                                </div>

                                                <span class="text-xs text-gray-400 w-6">{pIndex + 1}.</span>

                                                <input
                                                    type="text"
                                                    value={preset.name}
                                                    on:input={(e) => handlePresetNameChange(step.id, preset.id, e.currentTarget.value)}
                                                ...
                                                <div class="flex gap-1">
                                                    {#each ALL_INPUT_TYPES as type}
                                                        <button
                                                            ...
                                                            on:click={() => togglePresetType(step.id, preset.id, type)}
                                                ...
                                                <button
                                                    ...
                                                    on:click={() => removePreset(step.id, preset.id)}
                                                    title="Preset 삭제"
                                                >
                                                    ✕
                                                </button>
                                    ...
                                    <div class="p-3 bg-purple-50 rounded-lg border border-purple-200">
                                        <div class="flex items-center gap-3">
                                            <input
                                                ...
                                                class="flex-1 border border-gray-300 rounded px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-purple-500"
                                                on:keydown={(e) => e.key === 'Enter' && addPreset(step.id)}
                                            />
                                        </div>
                                        <div class="mt-2 flex items-center gap-2">
                                            <span class="text-xs text-gray-600">허용 형식:</span>
                                            {#each ALL_INPUT_TYPES as type}
                                                <button
                                                    class="px-2 py-1 text-xs rounded border transition
                                ...
                                {:else}
                                    <button
                                        ...
                                        on:click={() => { editingPresetStepId = step.id; newPresetName = ""; newPresetTypes = ['text']; }}
                                    >
                                        + Preset 추가
                                    </button>
                                {/if}
